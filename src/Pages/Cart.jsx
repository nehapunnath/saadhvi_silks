...
const Cart = () => {
  ...
  const handleQuantityChange = async (id, delta) => {
    try {
      ...
      await productApi.updateCartItem(id, newQuantity);
  ...
  const calculateSubtotal = () => cartItems.reduce((s, i) => s + i.price * i.quantity, 0);
  const calculateTotal = () => {
    const subtotal = calculateSubtotal();
    return subtotal + (subtotal > 5000 ? 0 : 250);
  };
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-[#F9F3F3] to-[#F7F0E8] py-12">
        <div className="container mx-auto px-4 text-center">
          <svg
            className="w-16 h-16 text-[#6B2D2D] mx-auto mb-6"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
          ...
          {/* <div className="mt-6">
            <Link to="/products" className="text-[#6B2D2D] font-medium hover:underline">
              Continue Shopping →
            </Link>
          </div> */}
        </div>
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-[#F9F3F3] to-[#F7F0E8] py-12">
        <div className="text-center py-20 bg-white rounded-3xl shadow-lg max-w-2xl mx-auto p-10">
          ...
          <Link to="/products" className="inline-block bg-[#6B2D2D] text-white px-8 py-3 rounded-full font-medium hover:bg-[#3A1A1A] transition shadow-md">
            Explore Collection
          </Link>
        </div>
      </div>
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-4 mt-3">
                          ...
                          <div className="flex items-center gap-2 bg-[#f9f3f3] rounded-full p-1">
                            ...
                            <button
                              onClick={() => handleQuantityChange(item.id, 1)}
                              className="w-9 h-9 flex items-center justify-center rounded-full bg-[#800020] text-white hover:bg-[#6B2D2D] transition"
                            >
                              +
                            </button>
                          </div>
